...
struct CustomTabCurve: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        return Path { path in
            ...
            path.addArc(tangent1End: p3, tangent2End: p4, radius: 36)
            path.addArc(tangent1End: p4, tangent2End: p1, radius: 36)
            
            let mid = curveAxis
            let curve = rect.height - 50
            path.move(to: CGPoint(x: mid - 58, y: height))
            
            // Curve
            let to1 = CGPoint(x: mid, y: curve)
            let control1 = CGPoint(x: mid - 29, y: height)
            let control2 = CGPoint(x: mid - 29, y: curve)
            path.addCurve(to: to1, control1: control1, control2: control2)
            
            // Joining Curve
            let to2 = CGPoint(x: mid + 58, y: height)
            let control3 = CGPoint(x: mid + 29, y: curve)
            let control4 = CGPoint(x: mid + 29, y: height)
